...
import { isSelectedSlotStillAvailable } from "@/lib/bookingCheckoutSteps.js";
...
export function ServiceBookingPanel({
  ...
  useEffect(() => {
    ...
    const addonDuration = addons.reduce((sum, addon) => {
      if (!addon.affects_duration) return sum;
      return sum + addon.duration_minutes * (s.addonQty[addon.id] ?? 0);
    }, 0);
    const baseDuration = (selectedPkg?.duration_minutes ?? null) ?? settings?.base_duration_minutes ?? 60;
    const total = (selectedPkg?.price ?? s.booking?.service?.base_price ?? 0) + addons.reduce(
      (sum, addon) => sum + addon.price * (s.addonQty[addon.id] ?? 0), 0
    );
    ...
    onStatusChange(slug, {
      ...
      display: {
        ...
        timezone: settings?.timezone || "America/Puerto_Rico",
        durationMinutes: baseDuration + addonDuration,
        total: total,
        currency: selectedPkg?.currency || s.booking?.service?.currency || "USD",
        addons: addons
