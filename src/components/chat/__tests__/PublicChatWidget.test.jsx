...
describe("PublicChatWidget — runtime visual público de AIRA", () => {
  it("expone un rail cerrado específico para mobile y un estado fullscreen al abrir", async () => {
    ...
    expect(await screen.findByLabelText(/aira invitando a abrir el chat/i)).toBeInTheDocument();
  ...
  it("mantiene stage, mensajes y composer como regiones hermanas sin handoff automático", async () => {
  ...
  it("mantiene el personaje del launcher fuera del botón y lo reemplaza por el stage al abrir", async () => {
    ...
    const launcher = screen.getByRole("button", { name: /abrir chat/i });
    const launcherCharacter = await screen.findByLabelText(/aira invitando a abrir el chat/i);
  ...
  it("alterna point-viewer e invite-chat únicamente en el personaje externo", async () => {
    vi.useFakeTimers();
  ...
  it("precarga y decodifica las poses críticas sin persistir sus URLs", async () => {
    ...
    expect(sessionStorage.getItem("aira_public_chat_history_v2") || "").not.toContain("cdn.example");
  });
...
describe("PublicChatWidget — estado inicial", () => {
  ...
  it("una sesión vieja de AIRA sin conversación real no gana sobre el default IVOX del CRM", async () => {
    sessionStorage.setItem("aira_public_chat_session_v2", JSON.stringify({
      session_id: "stale-aira-session",
      chatbot_key: "aira-webchat-public",
    }));
    sessionStorage.setItem("aira_public_chat_history_v2", JSON.stringify([]));
    getPublicChatDefaultAssistant.mockResolvedValueOnce({ key: "ivox-webchat-public", display_name: "IVOX" });
    getPublicAvatarRuntime.mockResolvedValueOnce(publicAvatarRuntime({
      profile: "ivox",
      default_pose: "point-viewer",
    ...
    expect(screen.queryByLabelText(/aira invitando a abrir el chat/i)).not.toBeInTheDocument();
    await waitFor(() => expect(screen.getByRole("button", { name: /abrir chat con ivox/i })).toBeInTheDocument());
    expect(screen.queryByLabelText(/aira invitando a abrir el chat/i)).not.toBeInTheDocument();
    expect(getPublicAvatarRuntime).toHaveBeenCalledWith({ chatbotKey: "ivox-webchat-public" });
  });
  ...
  it("presenta IVOX como AIRA: alterna point-viewer e invite-chat sin crossfade", async () => {
    vi.useFakeTimers();
    getPublicChatDefaultAssistant.mockResolvedValueOnce({ key: "ivox-webchat-public", display_name: "IVOX" });
    getPublicAvatarRuntime.mockResolvedValueOnce(publicAvatarRuntime({
      profile: "ivox",
    ...
    act(() => vi.advanceTimersByTime(1_100));
    expect(gestureImage).toHaveAttribute("src", "https://cdn.example/ivox-invite-chat.png");
  ...
  it("invalida una sesión v1 una sola vez y exige un nuevo pre-chat", () => {
    ...
    sessionStorage.setItem("aira_public_chat_quick_replies_v1", JSON.stringify({ session_id: "stale-session", options: [{ id: "old" }] }));
    sessionStorage.setItem("aira_public_chat_assistant_v1", "ivox-webchat-public");

    render(<PublicChatWidget />);

    expect(sessionStorage.getItem("aira_public_chat_storage_version")).toBe("2");
    expect(sessionStorage.getItem("aira_public_chat_session_v1")).toBeNull();
    expect(sessionStorage.getItem("aira_public_chat_history_v1")).toBeNull();
    expect(sessionStorage.getItem("aira_public_chat_quick_replies_v1")).toBeNull();
    expect(sessionStorage.getItem("aira_public_chat_assistant_v1")).toBeNull();
    expect(sessionStorage.getItem("aira_public_chat_session_v2")).toBeNull();
  });
...
describe("PublicChatWidget — H3B: polling de /events", () => {
  ...
  it("16) un 404 de /events detiene el polling, limpia sesión e historial, y vuelve a prechat", async () => {
    ...
    expect(JSON.parse(sessionStorage.getItem("aira_public_chat_history_v2") || "[]")).toEqual([]);
...
it("no renderiza un botón automático de handoff fuera de acciones explícitas", async () => {
  sessionStorage.setItem("aira_public_chat_session_v2", JSON.stringify({ session_id: "session-1", chatbot_key: "ivox-webchat-public" }));
  render(<PublicChatWidget />);
  openWidget();
  await vi.waitFor(() => expect(getPublicChatEvents).toHaveBeenCalled());
  expect(document.querySelector(".public-chat-widget__handoff-bar")).toBeNull();
