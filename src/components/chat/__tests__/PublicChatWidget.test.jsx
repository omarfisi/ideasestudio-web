...
describe("PublicChatWidget — runtime visual público de AIRA", () => {
  ...
  it("mantiene el personaje del launcher fuera del botón y lo reemplaza por el stage al abrir", async () => {
    ...
    await waitFor(() => expect(getPublicAvatarRuntime.mock.calls).toEqual(
      expect.arrayContaining([[{ profile: "ivox" }]])
    ));
    ...
    await waitFor(() => expect(getPublicAvatarRuntime.mock.calls).toEqual(
      expect.arrayContaining([[{ profile: "aira" }]])
    ));
    expect(await screen.findByRole("region", { name: /vista previa del avatar aira/i })).toBeInTheDocument();
  });
